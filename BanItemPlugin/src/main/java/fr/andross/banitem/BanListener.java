...
public final class BanListener {
    ...
    BanListener(@NotNull final BanItem plugin) {
        this.plugin = plugin;
    }
    ...
    public void load(@NotNull final CommandSender sender) {
        // Preparing variables
        final BanItemAPI api = plugin.getApi();
        final BanDatabase db = plugin.getBanDatabase();
        final Set<BanAction> priority = plugin.getBanConfig().getPriority();
        ...
        if (blacklist.contains(BanAction.ARMORSTANDPLACE) || whitelist) {
            if (!BanVersion.v8OrMore) {
                ...
                registerEvent(PlayerArmorStandManipulateEvent.class, (li, event) -> {
                    ...
                    if (Utils.isNullOrAir(e.getPlayerItem()))
        ...
        if (blacklist.contains(BanAction.ARMORSTANDTAKE) || whitelist) {
            if (!BanVersion.v8OrMore) {
                ...
                registerEvent(PlayerArmorStandManipulateEvent.class, (li, event) -> {
                    final PlayerArmorStandManipulateEvent e = (PlayerArmorStandManipulateEvent) event;
                    if (e.getArmorStandItem().getType() == Material.AIR)
                        return;
        ...
        if (blacklist.contains(BanAction.BREAK) || whitelist) {
            registerEvent(PlayerInteractEvent.class, (li, event) -> {
                ...
                if (e.getAction() == Action.LEFT_CLICK_BLOCK && e.getClickedBlock() != null) {
                    ...
                    if (api.isBanned(e.getPlayer(), e.getClickedBlock().getLocation(), e.getClickedBlock().getType(), true, BanAction.BREAK,
                            new BanData(BanDataType.MATERIAL, itemInHand.getType()))) {
                        e.setCancelled(true);
            ...
            registerEvent(BlockBreakEvent.class, (li, event) -> {
                if (!(event instanceof BlockBreakEvent))
                    return; // // also called for FurnaceExtractEvent...
                final BlockBreakEvent e = (BlockBreakEvent) event;
                final ItemStack itemInHand = Utils.getItemInHand(e.getPlayer());
                if (api.isBanned(e.getPlayer(), e.getBlock().getLocation(), e.getBlock().getType(), true, BanAction.BREAK,
                        new BanData(BanDataType.MATERIAL, itemInHand.getType()))) {
                    e.setCancelled(true);
                    if (!BanVersion.v12OrMore)
                        e.getPlayer().updateInventory();
        ...
        if (blacklist.contains(BanAction.BREW) || whitelist) {
            registerEvent(BrewEvent.class, (listener, event) -> {
                final BrewEvent brewEvent = (BrewEvent) event;
                final ItemStack ingridient = brewEvent.getContents().getIngredient() == null ? null : brewEvent.getContents().getIngredient().clone();
                final List<ItemStack> items = new ArrayList<>();
                for (final ItemStack item : brewEvent.getContents().getContents()) {
                ...
                plugin.getFoliaLib().getScheduler().runLater(() -> {
                    final BrewerInventory inv = brewEvent.getContents();
                    for (int i = 0; i < 3; i++) {
                        final ItemStack item = inv.getItem(i);
                        if (Utils.isNullOrAir(item))
                            continue;
                        if (api.isBanned(brewEvent.getBlock().getWorld(), item, BanAction.BREW)) {
                            if (!inv.getViewers().isEmpty())
                                if (!api.isBanned((Player) inv.getViewers().get(0), brewEvent.getBlock().getLocation(), item, true, BanAction.BREW))
                                    continue;
        ...
        if (blacklist.contains(BanAction.CONSUME) || whitelist) {
            registerEvent(PlayerItemConsumeEvent.class, (li, event) -> {
                ...
                if (Utils.isNullOrAir(e.getItem()))
                    return;
        ...
        if (blacklist.contains(BanAction.DELETE)) {
            registerEvent(InventoryOpenEvent.class, (ll, event) -> {
                ...
                plugin.getUtils().deleteItemFromInventoryView((Player) e.getPlayer());
        ...
        if (blacklist.contains(BanAction.DISPENSE) || whitelist) {
            registerEvent(BlockDispenseEvent.class, (li, event) -> {
                ...
                if (api.isBanned(e.getBlock().getWorld(), e.getItem(), BanAction.DISPENSE))
                    e.setCancelled(true);
        ...
        if (blacklist.contains(BanAction.DROPS) || whitelist) {
            if (BanVersion.v13OrMore)
                registerEvent(BlockDropItemEvent.class, (li, event) -> {
                    ...
                    final ItemStack itemInHand = Utils.getItemInHand(e.getPlayer());
                    e.getItems()
                            .removeIf(item -> api.isBanned(e.getPlayer(), e.getBlock().getLocation(), item.getItemStack(), true, BanAction.DROPS,
                                    new BanData(BanDataType.MATERIAL, itemInHand.getType())));
                ...
                registerEvent(BlockBreakEvent.class, (li, event) -> {
                    ...
                    if (e.getBlock()
                            .getDrops(itemInHand)
                            .stream()
                            .anyMatch(item -> api.isBanned(e.getPlayer(), e.getBlock().getLocation(), item, true, BanAction.DROPS,
                                    new BanData(BanDataType.MATERIAL, itemInHand.getType()))))
        ...
        if (blacklist.contains(BanAction.ENCHANT) || whitelist) {
            registerEvent(EnchantItemEvent.class, (li, event) -> {
                ...
                if (api.isBanned(e.getEnchanter(), e.getEnchantBlock().getLocation(), e.getItem(), true, BanAction.ENCHANT,
                        new BanData(BanDataType.ENCHANTMENT, EnchantmentWrapper.from(e.getEnchantsToAdd()))))
                    e.setCancelled(true);
            ...
            if (BanVersion.v9OrMore) {
                ...
                ItemStack denied;
                try {
                    denied = new ItemStackBuilder(plugin.getBanConfig().getConfig().getConfigurationSection("actions.enchant")).build();
                } catch (final Exception e) {
                    plugin.getUtils().sendMessage(sender, "&cUnable to load denied item 'actions.enchant' from config: " + e.getMessage());
                ...
                registerEvent(InventoryClickEvent.class, (li, event) -> {
                    ...
                    if (inventoryClickEvent.getView().getTopInventory().getType() != InventoryType.ANVIL)
                        return;

                    final ItemStack item = inventoryClickEvent.getInventory().getItem(0);
                    if (item == null)
                        return;
                    final ItemStack addedItem = inventoryClickEvent.getInventory().getItem(1);
                    if (addedItem == null)
                        return;
                    final Map<Enchantment, Integer> enchants = Utils.getAllEnchants(addedItem);
                    if (enchants.isEmpty())
                    ...
                    if (api.isBanned(player, item, true, BanAction.ENCHANT, new BanData(BanDataType.ENCHANTMENT, EnchantmentWrapper.from(enchants)))) {
                        inventoryClickEvent.getInventory().setItem(2, null);
                    ...
                    plugin.getFoliaLib().getScheduler().runAtEntity(player, (task) -> {
                        ...
                        if (item2 == null)
                            return;
                        final ItemStack addedItem2 = inventoryClickEvent.getInventory().getItem(1);
                        if (addedItem2 == null)
                            return;
            ...
            registerEvent(EntityDeathEvent.class, (li, event) -> {
                ...
                if (killer != null)
                    e.getDrops().removeIf(i -> {
                        if (i == null)
                            return false;
                        return api.isBanned(killer, e.getEntity().getLocation(), i, true, BanAction.ENTITYDROP,
                                new BanData(BanDataType.ENTITY, e.getEntity().getType()));
                    });
                else
                    e.getDrops().removeIf(i -> {
                        if (i == null)
        ...
        if (blacklist.contains(BanAction.ENTITYINTERACT) || whitelist) {
            if (BanVersion.v9OrMore)
                registerEvent(PlayerInteractEntityEvent.class, (li, event) -> {
                    ...
                    if (api.isBanned(e.getPlayer(), e.getRightClicked().getLocation(), item, true, BanAction.ENTITYINTERACT,
                            new BanData(BanDataType.ENTITY, e.getRightClicked().getType())))
                        e.setCancelled(true);
                }, priority.contains(BanAction.ENTITYINTERACT));
            else
                registerEvent(PlayerInteractEntityEvent.class, (li, event) -> {
                    final PlayerInteractEntityEvent e = (PlayerInteractEntityEvent) event;
                    if (api.isBanned(e.getPlayer(), e.getRightClicked().getLocation(), Utils.getItemInHand(e.getPlayer()), true, BanAction.ENTITYINTERACT,
                            new BanData(BanDataType.ENTITY, e.getRightClicked().getType())))
                        e.setCancelled(true);
                }, priority.contains(BanAction.ENTITYINTERACT));
        }
        ...
        if (blacklist.contains(BanAction.FILL) || whitelist) {
            registerEvent(PlayerBucketFillEvent.class, (li, event) -> {
                ...
                if (api.isBanned(e.getPlayer(), e.getBlockClicked().getLocation(), item, true, BanAction.FILL,
                        new BanData(BanDataType.MATERIAL, e.getBlockClicked().getType())))
                    e.setCancelled(true);
        ...
        if (blacklist.contains(BanAction.GLIDE) || whitelist) {
            if (!BanVersion.v9OrMore) {
                ...
                registerEvent(org.bukkit.event.entity.EntityToggleGlideEvent.class, (li, event) -> {
                    ...
                    if (item == null)
                        return;
                    if (api.isBanned(player, item, true, BanAction.GLIDE)) {
                        ...
                        plugin.getFoliaLib().getScheduler().runAtEntity(player, (task) -> {
                            ...
                            player.getInventory().setItem(freeSlot, chestplate);
                            ee.setChestplate(null);
                        });
                    }
        ...
        if (blacklist.contains(BanAction.HANGINGPLACE) || whitelist) {
            registerEvent(HangingPlaceEvent.class, (li, event) -> {
                ...
                if (e.getPlayer() == null)
                    return;
                final ItemStack item = Utils.getItemInHand(e.getPlayer());
                if (api.isBanned(e.getPlayer(), e.getEntity().getLocation(), item, true, BanAction.HANGINGPLACE,
                        new BanData(BanDataType.ENTITY, e.getEntity().getType())))
                    e.setCancelled(true);
        ...
        if (blacklist.contains(BanAction.HOLD) || whitelist) {
            ...
            registerEvent(InventoryClickEvent.class, (li, event) -> {
                ...
                if (!e.getView().getTopInventory().equals(e.getView().getBottomInventory()) &&
                        e.getView().getTopInventory().equals(Utils.getClickedInventory(e.getView(), e.getRawSlot())) && e.isShiftClick()) {
            ...
            if (BanVersion.v12OrMore) {
                registerEvent(org.bukkit.event.entity.EntityPickupItemEvent.class, (li, event) -> {
                    ...
                    if (toSlot == p.getInventory().getHeldItemSlot() &&
                            api.isBanned(p, e.getItem().getLocation(), e.getItem().getItemStack(), true, BanAction.HOLD))
                        e.setCancelled(true);
                }, priority.contains(BanAction.HOLD));
            } else {
        ...
        if (blacklist.contains(BanAction.INTERACT) || whitelist) {
            registerEvent(PlayerInteractEvent.class, (li, event) -> {
                ...
                if (e.getClickedBlock() != null && e.getAction() == Action.RIGHT_CLICK_BLOCK) {
                    final ItemStack item = Utils.getItemInHand(e.getPlayer());
                    if (api.isBanned(e.getPlayer(), e.getClickedBlock().getLocation(), e.getClickedBlock().getType(), true, BanAction.INTERACT,
                            new BanData(BanDataType.MATERIAL, item.getType()))) {
                        if (!BanVersion.v12OrMore)
                            e.getPlayer().updateInventory();
                        e.setCancelled(true);
                    }
        ...
        if (blacklist.contains(BanAction.INVENTORYCLICK) || whitelist) {
            registerEvent(InventoryClickEvent.class, (li, event) -> {
                final InventoryClickEvent e = (InventoryClickEvent) event;
                if (Utils.getClickedInventory(e.getView(), e.getRawSlot()) == null)
                    return;
        ...
        if (blacklist.contains(BanAction.PICKUP) || whitelist) {
            ...
            registerEvent(PlayerQuitEvent.class,
                    (li, event) -> plugin.getUtils().getMessagesCooldown().remove(((PlayerQuitEvent) event).getPlayer().getUniqueId()),
        ...
        if (blacklist.contains(BanAction.PLACE) || whitelist) {
            registerEvent(BlockPlaceEvent.class, (li, event) -> {
                ...
                if (Utils.isNullOrAir(e.getItemInHand()))
                    return;
        ...
        if (blacklist.contains(BanAction.USE) || whitelist) {
            registerEvent(PlayerInteractEvent.class, (li, event) -> {
                ...
                if (e.getAction() == Action.RIGHT_CLICK_BLOCK || e.getAction() == Action.RIGHT_CLICK_AIR) {
                    if (e.getClickedBlock() != null) {
                        if (api.isBanned(e.getPlayer(), e.getClickedBlock().getRelative(e.getBlockFace()).getLocation(), e.getItem(), true, BanAction.USE,
                                new BanData(BanDataType.MATERIAL, e.getClickedBlock().getType()))) {
                            e.setCancelled(true);
                            if (!BanVersion.v12OrMore)
                                e.getPlayer().updateInventory();
                        }
        ...
        if (blacklist.contains(BanAction.RENAME) || whitelist) {
            registerEvent(InventoryClickEvent.class, (li, event) -> {
                ...
                if (inv == null)
                    return;
                if (inv.getType() != InventoryType.ANVIL || e.getRawSlot() != 2)
                    return;
            ...
            final List<String> renameCommands = plugin.getBanConfig().getConfig().getStringList("actions.rename");
        ...
        if (blacklist.contains(BanAction.SMITH) || whitelist) {
            if (!BanVersion.v16OrMore) {
                ...
                registerEvent(PrepareSmithingEvent.class, (ll, event) -> {
                    if (!(event instanceof PrepareSmithingEvent))
                        return; // called for PrepareResultEvent too...
        ...
        if (blacklist.contains(BanAction.SWEEPINGEDGE) || whitelist) {
            if (!BanVersion.v12OrMore) {
                ...
                registerEvent(EntityDamageByEntityEvent.class, (li, event) -> {
                    ...
                    final Player damager = (Player) e.getDamager();
                    final ItemStack itemInHand = Utils.getItemInHand(damager);
                    if (api.isBanned(damager, e.getEntity().getLocation(), itemInHand, true, BanAction.SWEEPINGEDGE,
                            new BanData(BanDataType.ENTITY, e.getEntityType())))
                        e.setCancelled(true);
        ...
        if (blacklist.contains(BanAction.TRANSFER) || whitelist) {
            ...
            registerEvent(InventoryClickEvent.class, (li, event) -> {
                ...
                if (top.getType() != InventoryType.CRAFTING && e.getClick() == ClickType.DOUBLE_CLICK) { // Trying to get all items for a banned one?
                    ...
                    if (!Utils.isNullOrAir(item))
                        if (api.isBanned(p, item, true, BanAction.TRANSFER, new BanData(BanDataType.INVENTORY_FROM, bottom.getType()),
                                new BanData(BanDataType.INVENTORY_TO, top.getType()))) {
                            e.setCancelled(true);
                            return;
                        }
                }
                ...
                if (invClicked.equals(bottom)) { // Player Inventory clicked
                    ...
                    if (e.isShiftClick() && e.getAction() == InventoryAction.MOVE_TO_OTHER_INVENTORY) {
                        ...
                        if (Utils.isNullOrAir(item))
                            return;
                        if (api.isBanned(p, item, true, BanAction.TRANSFER, new BanData(BanDataType.INVENTORY_FROM, top.getType()),
                                new BanData(BanDataType.INVENTORY_TO, bottom.getType())))
                            e.setCancelled(true);
                    } else {
                        // Offhand click?
                        ...
                        if (e.getHotbarButton() > -1) {
                            ...
                            if (!Utils.isNullOrAir(hotBarItem) &&
                                    api.isBanned(p, hotBarItem, true, BanAction.TRANSFER, new BanData(BanDataType.INVENTORY_FROM, bottom.getType()))) {
                                e.setCancelled(true);
                                return;
                            }
                            ...
                            if (!Utils.isNullOrAir(item) &&
                                    api.isBanned(p, item, true, BanAction.TRANSFER, new BanData(BanDataType.INVENTORY_TO, bottom.getType()))) {
                                e.setCancelled(true);
                                return;
                            }
                        }
                        ...
                        if (!Utils.isNullOrAir(cursorItem)) {
                            if (api.isBanned(p, cursorItem, true, BanAction.TRANSFER, new BanData(BanDataType.INVENTORY_FROM, bottom.getType()),
                                    new BanData(BanDataType.INVENTORY_TO, top.getType()))) {
                                e.setCancelled(true);
                                return;
                            }
                        ...
                        if (!Utils.isNullOrAir(clickedItem)) {
                            if (api.isBanned(p, clickedItem, true, BanAction.TRANSFER, new BanData(BanDataType.INVENTORY_FROM, top.getType()),
                                    new BanData(BanDataType.INVENTORY_TO, bottom.getType())))
                                e.setCancelled(true);
                        }
                    }
            ...
            registerEvent(InventoryDragEvent.class, (li, event) -> {
                ...
                if (e.getView().getTopInventory().getType() == InventoryType.CRAFTING)
                    return;
                if (Utils.isNullOrAir(e.getOldCursor()))
                    return;
            ...
            // Hoppers block?
            if (plugin.getBanConfig().getConfig().getBoolean("actions.transfer.hoppers-block")) {
                ...
                registerEvent(InventoryPickupItemEvent.class, (li, event) -> {
                    ...
                    if (api.isBanned(e.getItem().getWorld(), e.getItem().getItemStack(), BanAction.TRANSFER,
                            new BanData(BanDataType.INVENTORY_TO, InventoryType.HOPPER)))
                        e.setCancelled(true);
                }, priority.contains(BanAction.TRANSFER));
            }
        }
        ...
        if (blacklist.contains(BanAction.UNFILL) || whitelist) {
            registerEvent(PlayerBucketEmptyEvent.class, (li, event) -> {
                ...
                final ItemStack item = Utils.getItemInHand(e.getPlayer());
                if (api.isBanned(e.getPlayer(), e.getBlockClicked().getLocation(), item, true, BanAction.UNFILL,
                        new BanData(BanDataType.MATERIAL, e.getBlockClicked().getType()))) {
                    e.setCancelled(true);
                    e.getPlayer().updateInventory();
                }
        ...
        if (blacklist.contains(BanAction.WEAR) || whitelist) {
            registerEvent(InventoryClickEvent.class, (li, event) -> {
                ...
                if (inventoryClickEvent.getInventory().getType() != InventoryType.PLAYER &&
                        inventoryClickEvent.getInventory().getType() != InventoryType.CRAFTING)
                    return;

                // Armor interaction?
                if (inventoryClickEvent.getRawSlot() >= 5 && inventoryClickEvent.getRawSlot() <= 8) {
                    plugin.getFoliaLib().getScheduler().runAtEntity(player, (task) -> plugin.getUtils().checkPlayerArmors(player));
                    return;
                }
                ...
                if (inventoryClickEvent.getRawSlot() >= 5 && inventoryClickEvent.getRawSlot() <= 8 && inventoryClickEvent.getHotbarButton() > -1) {
                    final ItemStack item = inventoryClickEvent.getView().getBottomInventory().getItem(inventoryClickEvent.getHotbarButton());
                    if (!Utils.isNullOrAir(item))
                        plugin.getFoliaLib().getScheduler().runAtEntity(player, (task) -> plugin.getUtils().checkPlayerArmors(player));
                }
            ...
            if (plugin.getBanConfig().getConfig().getBoolean("actions.wear.region-check") && plugin.getHooks().isWorldGuardEnabled()) {
            ...
            // Scanner?
            plugin.getUtils().getWearScanner().setEnabled(plugin.getBanConfig().getConfig().getBoolean("actions.wear.scanner"));
        }
    }
    ...
    private void registerEvent(@NotNull final Class<? extends Event> c, @NotNull final EventExecutor ee, final boolean priority) {
        Bukkit.getPluginManager().registerEvent(c, listener, (priority ? EventPriority.LOWEST : EventPriority.NORMAL), ee, plugin, !priority);
        activated++;
    }
